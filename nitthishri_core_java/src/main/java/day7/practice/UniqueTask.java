...
public class UniqueTask {
    ...
    public static void taskDuplicate(ArrayList<UniqueTask> tasks) {
        HashSet<UniqueTask> uniqueTasks = new HashSet<UniqueTask>(tasks);
        System.out.println(uniqueTasks);
    }
public static boolean validateDuplicatemethod(ArrayList<UniqueTask> tasks) throws IllegalArgumentException{
		
		HashSet<String> taskSet = new HashSet<>();
		for(UniqueTask task:tasks) {
